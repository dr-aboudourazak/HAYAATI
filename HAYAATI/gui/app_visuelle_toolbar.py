...
def configurer_bandeau_superieur(app):
    if hasattr(app, "barre_outils") and app.barre_outils:
        ...
        # ⚠️ 12/09/2026 : comparaison par sous-chaîne plutôt qu'égalité
        # stricte — page.platform est une énumération dans cette version
        # de Flet ("PagePlatform.ANDROID"), pas la chaîne "ANDROID" nue.
        # Ce cas précis était sauvé par le hasattr(sys, "getandroidapilevel")
        # en secours, mais autant corriger la vraie cause plutôt que de
        # compter sur un filet de sécurité qui masque le problème.
        plateforme = str(getattr(app.page, "platform", "") or "").upper()
        est_mobile = ("ANDROID" in plateforme or "IOS" in plateforme) or hasattr(sys, "getandroidapilevel")
    ...
    # 🆕 11/09/2026 : icône Réglages déplacée depuis le menu de navigation
    # vers le bandeau vert, à droite — accessible en mode visiteur ET
    # connecté désormais, alors qu'elle n'apparaissait qu'en mode connecté
    # dans le menu de navigation (voir dessiner_boutons_navigation, qui
    # n'incluait "reglages" que dans la liste app.mode_persistant_actif).
    elements_droite = []
    ...
    if app.est_mode_connecte:
        ...
        elements_droite.append(btn_mode)

    btn_reglages = ft.IconButton(
        icon=ft.Icons.SETTINGS,
        icon_color=ft.Colors.WHITE,
        icon_size=22,
        tooltip=barre_json.get("reglages_tooltip", "Réglages"),
        on_click=lambda _: app.basculer_ecran("REGLAGES"),
    )
    elements_droite.append(btn_reglages)

    cont.controls.append(
        ft.Container(
            content=ft.Row(elements_droite, spacing=6, alignment=ft.MainAxisAlignment.END),
...
def dessiner_boutons_navigation(app):
    ...
    if app.mode_persistant_actif:
        boutons = [
            ...
            (m_json.get("qiblah", "🕋 Prière"), "PRIERE"),

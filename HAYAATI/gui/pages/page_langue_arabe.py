...
class PageLangueArabe(ft.Container):
    ...
    def _action_langue(self, dic: dict):
        ...
        même si l'utilisateur est déjà sur cet écran (pas besoin de naviguer
        ailleurs et revenir pour voir le changement)."""
        self._rafraichir()
    ...
    def actualiser_donnees_affichage(self):
        ...
        if getattr(self.app, "demande_ouverture_caravane", False):
            ...
            etapes = (DICTIONNAIRE_LANGUES.actif.get("langue_arabe", {}) if DICTIONNAIRE_LANGUES.actif else self.contenu_complet.get("langue_arabe", {})).get("etapes", [])
    ...
    def construire_ecran(self):
        ...
        langue_arabe = (dic_actif.get("langue_arabe")
                         or (self.app.txt_global.get("langue_arabe", {}) if hasattr(self.app, "txt_global") else None)
                         or self.contenu_complet.get("langue_arabe", {}))
        self.txt_ui = (dic_actif.get("interface_savoir")
                        or (self.app.txt_global.get("interface_savoir", {}) if hasattr(self.app, "txt_global") else {})
                        or {})
        ...
        def construire_verso():
            ...
            lignes.append(ft.Row([
                ft.ElevatedButton("✅", bgcolor=VERT_SUCCES, color="white", height=32, width=48,
                                  ...
                                  tooltip=self.txt_ui.get("tooltip_je_connais", "Je la connais"), on_click=lambda e: marquer(e, True)),
                ft.ElevatedButton("❓❓", bgcolor=AMBRE_ATTENTE, color="white", height=32, width=48,
                                  style=ft.ButtonStyle(shape=ft.RoundedRectangleBorder(radius=6)),
                                  tooltip=self.txt_ui.get("tooltip_a_revoir", "À revoir"), on_click=lambda e: marquer(e, False)),
    ...
    def _bandeau_resume(self, resume):
        return ft.Container(
            content=ft.Text(f"✓ {resume['connues']} {self.txt_ui.get('resume_connues', 'connu(e)s')}   ·   🔁 {resume['a_revoir']} {self.txt_ui.get('resume_a_revoir', 'à revoir')}   ·   ⚪ {resume['non_vues']} {self.txt_ui.get('resume_non_vues', 'pas encore vu(e)s')}",
    ...
    def _construire_etape_alphabet(self, etape):
        ...
        for i, l in page_lettres:
            lignes = [
                ...
                (f"{self.txt_ui.get('label_translitteration', 'Translittération')} : {l['translitteration']}", 9, False),
                (f"{self.txt_ui.get('label_connexion', 'Connexion')} : {l['connexion']}", 9, False),
                (l.get("son", ""), 9, False),
            ]
    ...
    def _construire_etape_vocabulaire(self, etape):
        ...
        if not listes:
            return ft.Text(self.txt_ui.get("contenu_indisponible", "Contenu non disponible."), color=TEXTE_FONCE)
    ...
    def _construire_etape_conjugaison(self, etape):
        ...
        if regles_verbe:
            ...
            blocs_note.append(ft.Container(
                content=ft.Column([ft.Text(self.txt_ui.get("titre_profils_vocaliques", "Trois profils vocaliques à distinguer :"), size=11, weight=ft.FontWeight.BOLD, color=TERRACOTTA)] + cartes_regles, spacing=4, tight=True),
    ...
    def _construire_etape_grammaire(self, etape):
        ...
        if not lecons:
            contenu.append(ft.Text(self.txt_ui.get("aucune_lecon", "Aucune leçon."), color=TEXTE_FONCE))
            ...
            content=ft.Column(
                ...
                + [ft.Row([
                        ft.ElevatedButton("✅", bgcolor=VERT_SUCCES, color="white", height=36, width=52,
                                          style=ft.ButtonStyle(shape=ft.RoundedRectangleBorder(radius=6)),
                                          tooltip=self.txt_ui.get("tooltip_jai_compris", "J'ai compris"), on_click=lambda e: marquer(e, True)),
    ...
    def _construire_etape_lecture(self, etape):
        ...
        if texte.get("vocabulaire_nouveau"):
            vocab = [ft.Text(f"{self.txt_ui.get('label_vocabulaire_nouveau', 'Vocabulaire nouveau')} :", size=11, weight=ft.FontWeight.BOLD, color=TERRACOTTA)] + self._construire_bloc_exemples(texte.get("vocabulaire_nouveau", []))

        return ft.Column([
            self._bandeau_resume(resume),
            ft.Container(content=ft.Column([ft.Text(texte.get("titre", ""), size=14, weight=ft.FontWeight.BOLD, color=TERRACOTTA),
                                              ft.Text(f"{self.txt_ui.get('label_niveau', 'Niveau')} : {texte.get('niveau','')}", size=9, italic=True, color=TERRACOTTA)], spacing=2, tight=True),
                          bgcolor=TERRACOTTA_CLAIR, padding=10, border_radius=6, margin=marge(bottom=8)),
            *blocs_lignes,
            *vocab,
            ft.Row([
                ft.ElevatedButton("✅", bgcolor=VERT_SUCCES, color="white", height=36, width=52,
                                  style=ft.ButtonStyle(shape=ft.RoundedRectangleBorder(radius=6)),
                                  tooltip=self.txt_ui.get("tooltip_jai_lu", "J'ai lu ce texte"), on_click=lambda e: marquer(e, True)),
                ft.ElevatedButton("❓❓", bgcolor=AMBRE_ATTENTE, color="white", height=36, width=52,
                                  style=ft.ButtonStyle(shape=ft.RoundedRectangleBorder(radius=6)),
                                  tooltip=self.txt_ui.get("tooltip_a_relire", "À relire"), on_click=lambda e: marquer(e, False)),
            ], alignment=ft.MainAxisAlignment.CENTER, spacing=10),
    ...
    def _construire_etape_caravane(self, etape):
        ...
        dic_actif = DICTIONNAIRE_LANGUES.actif or {}
        contenu_langue = dic_actif.get("langue_arabe") or self.contenu_complet.get("langue_arabe", {})
        contenu_sciences = dic_actif.get("sciences_islamiques") or self.contenu_complet.get("sciences_islamiques", {})
        ...
        if not session:
            return ft.Container(
                content=ft.Column([
                    ft.Text(self.txt_ui.get("msg_caravane_vide_titre", "🌱 Pas encore de quoi remplir une caravane"), size=14, weight=ft.FontWeight.BOLD, color=OCRE),
                    ft.Text(self.txt_ui.get("msg_caravane_vide_corps", "Parcourez l'alphabet, le vocabulaire, la conjugaison, la grammaire ou les sciences islamiques — la Caravane se remplira automatiquement."), size=11, color=TEXTE_FONCE),
                ], spacing=6),
                bgcolor=OCRE_CLAIR, padding=14, border_radius=6,
            )
        ...
        badge = self.txt_ui.get("badge_a_consolider", "🔁 À consolider") if not arret.get("complement") else self.txt_ui.get("badge_deja_connu", "✓ Déjà connu — petit rappel")
        return ft.Column([
            ft.Container(content=ft.Text(f"🧭 {nb_total} {self.txt_ui.get('lbl_points_a_revoir_total', 'point(s) à revoir au total')} — {len(session)} {self.txt_ui.get('lbl_etapes_dans_caravane', 'étape(s) dans cette caravane')}", size=11, weight=ft.FontWeight.BOLD, color=TERRACOTTA),
            ...
            ft.Container(
                content=ft.Column([
                    ...
                    ft.Row([
                        ft.ElevatedButton("✅", bgcolor=VERT_SUCCES, color="white", height=36, width=52,
                                          style=ft.ButtonStyle(shape=ft.RoundedRectangleBorder(radius=6)),
                                          tooltip=self.txt_ui.get("tooltip_je_maitrise", "Je la maîtrise"), on_click=lambda e: marquer(e, True)),
                        ft.ElevatedButton("❓❓", bgcolor=AMBRE_ATTENTE, color="white", height=36, width=52,
                                          style=ft.ButtonStyle(shape=ft.RoundedRectangleBorder(radius=6)),
                                          tooltip=self.txt_ui.get("tooltip_encore_a_revoir", "Encore à revoir"), on_click=lambda e: marquer(e, False)),
                    ], alignment=ft.MainAxisAlignment.CENTER, spacing=10),
                ], spacing=6, tight=True),

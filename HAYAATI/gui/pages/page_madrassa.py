...
class PageMadrassa(ft.Container):
    ...
    def _progres_encyclopedie(self, m_hub):
        ...
        if not perles:
            return m_hub.get("lbl_pas_commence", "Pas encore commencé")
        return f"✨ {perles} {m_hub.get('lbl_perles', 'perle(s)')}"

    def _progres_module_langue(self, cle_module, m_hub):
        u_id = getattr(self.app, "user_id_connecte", None)
        if not u_id or not hasattr(self.app, "sync_engine"):
            return None
        ...
        if not connues and not a_revoir:
            return m_hub.get("lbl_pas_commence", "Pas encore commencé")
        libelle = f"✓ {connues} {m_hub.get('lbl_acquis', 'acquis')}"
        if a_revoir:
            libelle += f"  ·  ❓❓ {a_revoir}"
        return libelle
    ...
    def construire_interface(self):
        ...
        grille.controls.append(self._carte_module(
            "🗺️", m_hub.get("titre_caravane", "Caravane du Savoir"),
            m_hub.get("desc_caravane", "Révision groupée : Langue arabe + Sciences islamiques"),
        ...
        if not getattr(self.app, "user_id_connecte", None):
            self.zone_contenu.controls.append(ft.Container(
                content=ft.Text(m_hub.get("msg_mode_visiteur", "👤 Mode Visiteur : la progression n'est pas conservée d'une session à l'autre."),

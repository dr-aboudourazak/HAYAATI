...
def compiler_et_dessiner_les_3_carrousels(instance_onboarding, txt_cal_complet, txt_onb):
    ...
    if zone is None or zone not in instance_onboarding.layout_principal.controls:
        ...
        # ⚠️ 12/09/2026 : les deux lignes qui retiraient cadre_calendrier ici
        # ont été supprimées. C'était un reliquat d'une époque où rien ne
        # reconstruisait le calendrier — le retirer était alors le bon
        # réflexe de nettoyage. Depuis que page_onboarding.py le
        # reconstruit à chaque rafraîchissement (juste avant cet appel),
        # ce nettoyage l'effaçait systématiquement avant même que le
        # rendu final n'ait lieu : le calendrier semblait "ne jamais
        # s'afficher" alors qu'il était bien construit et ajouté, puis
        # retiré quelques lignes plus tard, à chaque fois.

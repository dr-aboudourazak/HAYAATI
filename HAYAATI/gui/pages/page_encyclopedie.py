...
class PageEncyclopedie(ft.Container):
    ...
    def _construire_vue_catalogue(self, txt, categories):
        ...
        self._construire_bandeau_progression(categories, progres, txt)
    ...
    def _construire_bandeau_progression(self, categories, progres, txt):
        ...
        cle_badge, libelle_badge_repli = badge_actuel(perles)
        # Les libellés de badges ("Apprenti du Savoir", etc.) vivent dans le moteur
        # jeu_educatif_engine.py uniquement comme repli français : la traduction
        # réelle se fait ici via la clé (cle_badge), dans dic["encyclopedie"]["badges"].
        badges_json = DICTIONNAIRE_LANGUES.actif.get("encyclopedie", {}).get("badges", {})
        libelle_badge = badges_json.get(cle_badge, libelle_badge_repli)
        ...
        if suite:
            manquantes, cle_prochain, prochain_libelle_repli = suite
            prochain_libelle = badges_json.get(cle_prochain, prochain_libelle_repli)
            self.zone_contenu.controls.append(ft.Container(
                content=ft.Text(txt.get("lbl_encore_x_bonnes_reponses", "Encore {} bonne(s) réponse(s) avant « {} »").format(manquantes, prochain_libelle), size=10, color=GRIS_TEXTE),
        ...
        if nb_questions:
            if index in self.quiz_visible:
                self.zone_contenu.controls.append(ft.ElevatedButton(
                    f"🎯 {txt.get('lbl_masquer_quiz', 'Masquer le quiz')}", bgcolor=accent, color=BLANC,
                ...
                self.zone_contenu.controls.append(ft.ElevatedButton(
                    f"🎯 {txt.get('lbl_quiz', 'Quiz')} ({nb_questions})", bgcolor=accent, color=BLANC,
                    style=ft.ButtonStyle(shape=ft.RoundedRectangleBorder(radius=8)),
                    on_click=lambda e: self._basculer_quiz(index),
                ))

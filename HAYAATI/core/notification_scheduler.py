...
class NotificationScheduler:
    ...
    # ⏱ 08/09/2026, ajusté 09/09/2026 : nombre de minutes avant l'heure
    # légale de la prière auxquelles l'alarme adhan sonne. Lu depuis
    # AgendaEngine.FENETRE_RAPPEL_MINUTES au chargement du module — source
    # unique partagée avec le bandeau d'avertissement de
    # page_onboarding_alerts.py. Si AgendaEngine est indisponible pour
    # une raison quelconque, 5 minutes de secours (alignées sur la valeur
    # courante d'AgendaEngine, pas un chiffre arbitraire différent).
    RAPPEL_AVANT_PRIERE_MINUTES: int = (
        AgendaEngine.FENETRE_RAPPEL_MINUTES if _HAS_AGENDA_ENGINE else 5
    )
    ...
    async def schedule_prayer_period(self, days: int = 30) -> int:
        ...
        for delta in range(days):
            ...
            for idx, (cle, (nom, prayer_key)) in enumerate(priere_map.items()):
                ...
                # ⚠️ 08/09/2026 : l'alarme sonne désormais avant l'heure
                # légale (principe de l'adhan), pas exactement à l'heure.
                # RAPPEL_AVANT_PRIERE_MINUTES vient d'AgendaEngine, la même
                # constante qui pilote le bandeau d'avertissement sur
                # onboarding — les deux basculent ensemble si elle change.
                heure_rappel = heure - timedelta(minutes=self.RAPPEL_AVANT_PRIERE_MINUTES)

                # Si la fenêtre de rappel est déjà passée mais que la
                # prière elle-même n'a pas encore eu lieu (ex. programmation
                # relancée en retard dans la journée), on sonne tout de
                # suite plutôt que de rater complètement le rappel du jour.
                moment_declenchement = heure_rappel if heure_rappel > now else now

                if moment_declenchement < heure:
                    notif_id = self._stable_prayer_id(date_adhan, idx)
                    nom_traduit = noms_i18n.get(prayer_key, nom)
                    title = txt_onb.get(
                        "alarme_priere_titre_avant", "🕌 {} dans quelques minutes"
                    ).format(nom_traduit)
                    body = txt_onb.get(
                        "alarme_priere_body_avant", "La prière de {} approche."
                    ).format(nom_traduit)
                    ok = await self.schedule_prayer_alarm(
                        notif_id=notif_id,
                        title=title,
                        body=body,
                        when=moment_declenchement,
                        prayer_key=prayer_key,
                    )
                    if ok:
                        count += 1

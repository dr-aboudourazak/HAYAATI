...
# Le libellé ici sert de repli si la traduction JSON est absente pour une
# langue donnée — l'affichage réel doit passer par la clé (cle) pour être
# traduit dynamiquement (voir page_encyclopedie.py, dic["encyclopedie"]["badges"]).
PALIERS_BADGES = [
...
def badge_actuel(nb_perles):
    """Renvoie (cle_badge, libelle_repli) correspondant au nombre de perles obtenues.
    libelle_repli est le texte français codé en dur ici, à n'utiliser que si aucune
    traduction n'est trouvée dans le JSON de langue active pour cle_badge."""
    resultat = (PALIERS_BADGES[0][1], PALIERS_BADGES[0][2])
...
def prochain_palier(nb_perles):
    """Renvoie (perles_manquantes, cle_prochain_badge, libelle_repli) ou None si le
    dernier palier est atteint. cle_prochain_badge permet à l'appelant de retrouver
    le libellé traduit dans le JSON ; libelle_repli est le texte français de secours."""
    for seuil, cle, libelle in PALIERS_BADGES:
        if nb_perles < seuil:
            return seuil - nb_perles, cle, libelle
    return None

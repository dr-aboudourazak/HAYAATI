...
def obtenir_evenement_hegiri(mois_h, jour_h):
    """Détecte les grandes dates du calendrier sunnite et renvoie des clés techniques i18n."""
    ...
    if jour_h in [13, 14, 15] and mois_h != 9: 
        ...
        if not (mois_h == 12 and jour_h == 13):
            return "evt_jours_blancs"

    if mois_h == 9 and jour_h >= 25:
        return "evt_fin_ramadan"
    
    if mois_h == 12 and 1 <= jour_h <= 9:
        if jour_h == 9:
            return "evt_arafa"
        # Ajout du jour dynamique à la clé pour permettre la traduction composite (ex: "evt_dhu_hijjah_1")
        return f"evt_dhu_hijjah_{jour_h}"
    
    if mois_h == 12 and jour_h == 10:
        return "evt_aid_adha"
    
    if mois_h == 1 and jour_h == 10:
        return "evt_achoura"

...
class PrayerTimes:
    ...
    def _sun_declination(jd: float) -> Tuple[float, float, float]:
        """Retourne (déclinaison en radians, équation du temps en minutes, longitude solaire en radians).

        ⚠️ CORRECTION 09/09/2026 : la ligne de calcul de L mélangeait des
        radians et des degrés. q était déjà converti en radians
        (`q = math.radians(...)`) puis additionné à
        `math.degrees(1.915 * math.sin(g))` — or `1.915 * math.sin(g)`
        est déjà une valeur en degrés (1.915° est la constante de
        l'équation du centre), donc lui appliquer math.degrees() la
        multipliait par 57.3 en trop. L'ensemble (radians + degrés mal
        rééchelonnés) était ensuite réduit modulo 360 et reconverti en
        radians une seconde fois. Résultat : une équation du temps
        pouvant atteindre plusieurs centaines de minutes au lieu de
        rester dans l'intervalle physique réel (±17 minutes maximum sur
        l'année), et des horaires de prière calculés n'importe où sur
        les 24 heures. Confirmé numériquement : l'ancien code donnait
        eqt ≈ -384 min pour le 8 septembre 2026, le nouveau donne
        eqt ≈ +2,3 min, cohérent avec les tables astronomiques réelles
        pour cette période de l'année.

        Correction : tout est maintenant tenu en degrés (g_deg, q_deg,
        L_deg, e_deg) jusqu'à la conversion en radians, faite une seule
        fois par valeur, juste avant les appels trigonométriques qui
        l'exigent.
        """
        d = jd - 2451545.0
        ...
        # 1.915*sin(g) et 0.020*sin(2g) sont déjà en degrés (constantes de
        # l'équation du centre) — pas de math.degrees() ici, justement
        # l'erreur d'origine.
        L_deg = (q_deg + 1.915 * math.sin(g) + 0.020 * math.sin(2 * g)) % 360
        L = math.radians(L_deg)
        e_deg = 23.439 - 0.00000036 * d
        e = math.radians(e_deg)
        RA = math.degrees(math.atan2(math.cos(e) * math.sin(L), math.cos(L)))
        RA = (RA + 360) % 360
        eqt = (q_deg - RA) * 4  # 1° d'écart = 4 minutes de temps
    ...
    def get_times(self, d: date, method: Dict = CalculationMethod.MWL, asr_hanafi: bool = False) -> Dict[str, str]:
        ...
        # ⚠️ CORRECTION 09/09/2026 : asr_angle est l'ALTITUDE du soleil
        # au-dessus de l'horizon au moment de l'Asr (~40-45° typiquement),
        # une valeur positive — contrairement à Fajr/Isha où l'angle
        # représente une dépression SOUS l'horizon (d'où le signe négatif
        # dans leurs appels à _hour_angle). Le code appelait
        # _hour_angle(decl, -asr_angle), calculant l'heure à laquelle le
        # soleil est à 42° SOUS l'horizon (donc en pleine nuit) au lieu de
        # 42° AU-DESSUS. Résultat observé : Asr calculé après Maghrib et
        # Isha le même jour. Confirmé numériquement pour lat=11.10,
        # lon=0.03 le 8 septembre 2026 : version buguée → Asr 20:58,
        # version corrigée → Asr 15:08, cohérent avec Dhuhr 11:57 et
        # Maghrib 18:05 encadrant correctement l'Asr.

...
Version 5.4 - Shield Maître Anti-Fuite du Radd Partiel et Canalisation Canonique du Bayt Al-Mâl
...
def ajuster_proportions_sharia(dictionnaire_fractions_base, doctrine_active="Malikite"):
    ...
    # --- CAS B : LE RADD (Somme des parts fixes < 1.0) ---
        ...
        heritiers_de_sang = [h for h in fractions_ajustees.keys() if h not in ["epouse", "epoux", "###", "بيت مال المسلمين"]]
        ...
        if heritiers_de_sang:
            ...
            # On laisse le code s'exécuter pour que le bouclier final capture la fuite vers le Trésor Public.
    ...
    # =========================================================================
    # 🌟 LE COUSSIN DE SÉCURITÉ ARITHMÉTIQUE DE SECOURS (CANONISÉ EN ARABE DUR)
    # =========================================================================
    # Si le budget n'est toujours pas bouclé à 1.0 (cas Malikite sans Radd, ou anomalie),
    # la clé sacrée intercepte instantanément l'excédent pour verrouiller la balance à 100%.
    ...
    if reliquat_absolu_restant > 0.0001:
        # 🎯 RECTIFICATION CHIRURGICALE : Substitution du jeton technique par l'écriture sacrée
        fractions_ajustees["بيت مال المسلمين"] = fractions_ajustees.get("بيت مال المسلمين", 0.0) + reliquat_absolu_restant

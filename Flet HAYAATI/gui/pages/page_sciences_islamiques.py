...
class PageSciencesIslamiques(ft.Container):
    def __init__(self, app_reference):
        ...
        self.contenu = charger_sciences_islamiques()
    ...
    def construire_page(self):
        ...
        if not lecons:
            self.zone_contenu.controls.append(ft.Text("Aucune leçon.", color=TEXTE_FONCE))
            return
        ...
        pied = ft.Row([
            ft.ElevatedButton("✓ J'ai compris", bgcolor=VERT_SUCCES, color="white", height=32,
                              ...
                              on_click=lambda _: marquer(None, True)),
            ft.ElevatedButton("🔁 À revoir", bgcolor=AMBRE_ATTENTE, color="white", height=32,
                              style=ft.ButtonStyle(shape=ft.RoundedRectangleBorder(radius=4)),
                              on_click=lambda _: marquer(None, False)),
        ], spacing=8)
        ...
        barre_pagination = ft.Row([
            ft.ElevatedButton("◂ Précédent", on_click=lambda e: naviguer(-1), disabled=(etat.index_lecon == 0),
                              style=ft.ButtonStyle(shape=ft.RoundedRectangleBorder(radius=4))),
            ft.Text(f"{etat.index_lecon + 1} / {len(lecons)}", size=11, color=TEXTE_FONCE),
            ft.ElevatedButton("Suivant ▸", on_click=lambda e: naviguer(1), disabled=(etat.index_lecon >= len(lecons) - 1),
                              style=ft.ButtonStyle(shape=ft.RoundedRectangleBorder(radius=4))),

"""
HUB CENTRAL DE L'INVENTAIRE ET DES POSSESSIONS (GUI/PAGES/PAGE_INVENTAIRE.PY)
Version 5.5 Finale Unifiée - Cartes Multi-Accès & Double Graphique Vectoriel Audit (Zéro Encyclopédie)
"""
...
class PageInventaire(ft.Container):
    def __init__(self, app_reference):
        ...
        self.lbl_patrimoine = ft.Text(size=14, weight=ft.FontWeight.BOLD, color="#064e3b")
        self.lbl_famille = ft.Text(size=14, weight=ft.FontWeight.BOLD, color="#064e3b")
        self.lbl_testament = ft.Text(size=14, weight=ft.FontWeight.BOLD, color="#064e3b")

        # --- 2. GRILLE RESPONSIVE POUR ACCUEILLIR LES 3 CARTES MAÎTRESSES ---
        self.grille_inventaire = ft.ResponsiveRow(spacing=15, run_spacing=15)
        
        def fabriquer_carte_inventaire(emoji_visuel: str, lbl_control: ft.Text, ecran_cible: str, bg_color: str):
            return ft.Container(
                content=ft.Column([
        ...
        # Raccordement physique étanche vers vos 3 sous-pages existantes
        self.grille_inventaire.controls.extend([
            fabriquer_carte_inventaire("💵", self.lbl_patrimoine, "FINANCES", "#f9fafb"),
            fabriquer_carte_inventaire("👥", self.lbl_famille, "ARBRE", "#f0fdf4"),
            fabriquer_carte_inventaire("📜", self.lbl_testament, "TESTAMENT", "#fffbeb")
        ])
        ...
        # --- 4. ASSEMBLAGE MAÎTRE : Grille de cartes au-dessus, Graphiques au-dessous ---
        self.layout_total = ft.Column([
            self.lbl_titre_hub,
            ft.Container(height=5),
            self.grille_inventaire,   # 🌟 Vos 3 cartes d'accès direct
            ft.Container(height=15),
            self.grille_graphes       # 🌟 Vos 2 graphiques vectoriels
    ...
    def traduire_page(self, dic: dict):
        ...
        self.lbl_titre_hub.value = str(m_json.get("inventaire_hub", "Mon Inventaire")).strip()
        
        self.lbl_patrimoine.value = str(m_json.get("finances", "Patrimoine")).replace("💰", "").replace("💵", "").strip()
        self.lbl_famille.value = str(m_json.get("arbre", "Famille")).replace("👥", "").strip()
        self.lbl_testament.value = str(m_json.get("testament", "Testament")).replace("📝", "").replace("📜", "").strip()

...
class PageProfil(ft.Container):
    def __init__(self, app_reference):
        ...
        for c in self.cles_champs:
            ...
            if c == "birth":
                # 🎯 SÉCURISATION DU SÉLECTEUR DE DATE : Remplacement par 3 Dropdowns compacts Flet
                self.cb_jour = ft.Dropdown(
                    options=[ft.dropdown.Option(f"{i:02d}") for i in range(1, 32)],
                    width=82, height=38, text_size=12, value="07", dense=True, bgcolor=ft.Colors.WHITE
                )
                self.cb_mois = ft.Dropdown(
                    options=[ft.dropdown.Option(f"{i:02d}") for i in range(1, 13)],
                    width=82, height=38, text_size=12, value="04", dense=True, bgcolor=ft.Colors.WHITE
                )
                self.cb_annee = ft.Dropdown(
                    options=[ft.dropdown.Option(str(i)) for i in range(1930, 2027)],
                    width=95, height=38, text_size=12, value="1988", dense=True, bgcolor=ft.Colors.WHITE
                )
                ...
                cellule_formulaire = ft.Container(
                    content=ft.Column([self.labels[c], self.c_date_triple], spacing=4), col={"xs": 12, "md": 6}
                )
                ...
                self.entries[c] = ft.TextField(
                    value="-", height=40, text_size=13, password=est_masque, can_reveal_password=est_masque,
                    border_radius=6, border_color=ft.Colors.GREY_400, bgcolor=ft.Colors.WHITE
                )
                setattr(self, f"en_{c}", self.entries[c])
                
                cellule_formulaire = ft.Container(
                    content=ft.Column([self.labels[c], self.entries[c]], spacing=4), col={"xs": 12, "md": 6}
                )
                self.grille_profil.controls.append(cellule_formulaire)

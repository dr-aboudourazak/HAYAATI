...
class EcranZakat(ft.Container):
    ...
    # ============================================================
    # 🎯 PARTIE 2/3 : ALGORITHME SHARIA LIVE ET ACCÈS CERTIFICAT PDF (CORRIGÉE)
    # ============================================================
    ...
    def actualiser_contexte(self):
        ...
        txt_fin_bloc = l_active.get("finances", {}) if l_active else {}
        ...
        try:
            ...
            # 🎯 TRADUCTION DYNAMIQUE DE L'ARBITRAGE DU NISAB ET DE LA DOCTRINE BIJOUX
            ref_metal = res.get("metal_seuil_reference", "OR")
            if "OR" in str(ref_metal).upper():
                lbl_m_ref = txt_zk.get("options_nisab", {}).get("nisab_or", "Or (85g)")
            elif "ARGENT" in str(ref_metal).upper():
                lbl_m_ref = txt_zk.get("options_nisab", {}).get("nisab_argent", "Argent (595g)")
            else:
                lbl_m_ref = txt_zk.get("options_nisab", {}).get("nisab_plus_bas", "Plus bas")

            self.nisab_affiche_ecran = lbl_m_ref

            cle_bijoux = res.get("statut_doctrine_or")
            txt_doctrine_bijoux = txt_zk.get("bijoux_inclus", "Inclus") if cle_bijoux == "bijoux_inclus" else txt_zk.get("bijoux_exemptes", "Exempté")

            # CALCUL SHARIA DES Poids EN GRAMMES RÉELS DEPUIS SQLITE POUR L'UTILISATEUR CONNECTÉ
            if doctrine == "Hanafite":
                p_or_poids = float(cache_fin.get("or_refuge_poids", 0.0)) + float(cache_fin.get("or_parure_poids", 0.0))
                p_ag_poids = float(cache_fin.get("argent_refuge_poids", 0.0)) + float(cache_fin.get("argent_parure_poids", 0.0))
            else:
                p_or_poids = float(cache_fin.get("or_refuge_poids", 0.0))
                p_ag_poids = float(cache_fin.get("argent_refuge_poids", 0.0))
            
            # CONFIGURATION ET RE-CONSTRUCTION DU DICTIONNAIRE MULTILINGUE POUR LE TABLEAU PDF 1
            self.intrants_mem = {
                "LIQUIDE": f"{float(cache_fin.get('liq', 0.0)):.2f} {dev}",
                "STOCK": f"{stock_live:.2f} {dev}",
            ...
            v += f"   • {txt_zk.get('nisab_applique', 'Nisab')} : {txt_doctrine_bijoux}\n"
    ...
    # ============================================================
    # 🎯 PARTIE 3/3 : ALGORITHME SHARIA LIVE ET ACCÈS CERTIFICAT PDF
    # ============================================================

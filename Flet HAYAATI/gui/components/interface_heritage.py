...
class EcranHeritage(ft.Container):
    ...
    def actualiser_contexte(self):
        ...
        if self.personnes_exclues_mem:
            for ex in self.personnes_exclues_mem: 
                ...
                # Gestion de la racine de la clé pour neutraliser les index de multiplicité (ex: epouse_1 -> epouse)
                cle_json_base = cle_json.split("_")[0] if "_" in cle_json and any(str(i) in cle_json for i in range(10)) else cle_json
                
                # 🎯 LE FILTRE ANTI-DOUBLON UNIVERSEL GÉNÉRIQUE :
                # Si la clé est active en haut (y compris notre Asabah stabilisé à 0.0), elle disparait du bas
        ...
        # Injection de la chaîne finale de manière réactive
    ...
    def imprimer_pdf_heritage_live(self):
        ...
        # Si le régime est actif, on injecte les effectifs réels d'oncles et tantes utérins.
        # Pour faire apparaître la mention "Zawil Arham" en arabe sans faire planter le traducteur JSON,
        # on utilise le canal de l'Époux (absent à 100% dans cette situation) et on remplace sa clé 
        # par l'écriture sacrale. Votre CertificateEngine affichera la ligne dans le cadre vert !
        if is_arham_actif:
            try:
                ...
                # Le double ancrage : Le père à 0 ouvre le cadre, et l'écriture arabe s'impose
                saisi["pere"] = 0
                saisi["مسألة ذوي الأرحام"] = 1
                
                # On passe le témoin d'activation standard pour votre moteur au cas où
        ...
        if is_khountha_actif:
            ...
            # 🎯 PIVOT DE SÉCURITÉ UNIVERSEL HOUSTAZ : Force ReportLab à ouvrir et 
            # dessiner le cadre vert supérieur même s'il n'y a pas de parents/enfants directs !
            saisi["pere"] = 0 

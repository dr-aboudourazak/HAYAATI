...
class EcranHeritageTiers(ft.Container):
        ...
        try:
            ...
            if self.lignes_exclus_mem:
                for ex in self.lignes_exclus_mem:
                    cle_brute = str(ex).strip().split(":", 1).strip().upper()
                    ...
                    cle_json_base = cle_json.split("_") if "_" in cle_json and any(str(i) in cle_json for i in range(10)) else cle_json
    ...
    # ============================================================
    # 🎯 PARTIE 4/4 : HARMONISATION DU RAPPORT ÉCRAN ET SYSTÈME INTRANTS PDF
    # ============================================================
        ...
        if self.ventilation:
            for heritier, fraction in self.ventilation.items():
                # 🎯 HARMONISATION BAS PDF TIERS : Remplacement par l'arabe littéraire pour le Khountha
                if str(heritier).lower() == "khountha":

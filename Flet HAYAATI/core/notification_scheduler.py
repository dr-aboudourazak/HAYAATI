...
Version 1.0 — Programme automatiquement les 5 prières et événements lunaires.
...
# ---------------------------------------------------------------------------
try:
    from adhan import Coordinates, PrayerTimes, CalculationMethod, Madhab
    _HAS_ADHAN = True
except ImportError:
    _HAS_ADHAN = False
    print("[SCHEDULER] WARNING: 'adhan' non installé. pip install adhan")
...
class NotificationScheduler:
    ...
    def schedule_prayer_period(self, days: int = 30) -> int:
        ...
        """
        if not _HAS_ADHAN:
            print("[SCHEDULER] ERREUR: pip install adhan requis.")
            return 0
